 ...
 for item in items+[{'name':'BowVertical'},{'name':'BowHorizontal'}]:
  ...
  with writer(dest) as clip:
   for p in sorted((frames/n).glob('frame_*.png')):
    im=Image.open(p).convert('RGB');d=ImageDraw.Draw(im);d.rectangle((0,0,640,44),fill='black');d.text((12,6),labels[n]+' | 관통 수정 완료',fill='white',font=font);a=np.asarray(im);clip.append_data(a);total.append_data(a)
...
with zipfile.ZipFile(root/'Combat_Animations_Package.zip','w',zipfile.ZIP_DEFLATED) as z:
 ...
 for n in ['bow_dynamic_verification.json','bow_export_verification.json','bow_collision_final.json']:z.write(work/n,'Gukgung/'+n)
